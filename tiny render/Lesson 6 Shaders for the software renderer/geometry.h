...
template <typename T> struct vec<3, T> 
{
	...
	const T& operator[](const size_t i) const { assert(i < 3); 
...
// projection则是“将4D再变回3D”，或者说，投影回3D
...
template<size_t DimRows, size_t DimCols, typename T> class mat 
{
	...
	vec<DimCols, T> rows[DimRows]; // rows[DimRows]——用来记录每一个顶点的u v坐标 
public:
	mat() {}
