...
template<size_t DIM, typename T> T operator*(const vec<DIM, T>& lhs, 
					const vec<DIM, T>& rhs) 
{
	T ret = T();
...
template<size_t DIM, typename T, typename U> vec<DIM, T> operator*(vec<DIM, T> lhs, 
						const U& rhs) 
{
...
template<size_t LEN, size_t DIM, typename T> vec<LEN, T> embed(const vec<DIM, T>& v, 
					T fill = 1) 
{
	vec<LEN, T> ret;
...
template <size_t DIM, typename T> std::ostream& operator<<(std::ostream& out, 
							vec<DIM, T>& v)
{
...
public:
	...
	mat<DimRows, DimCols, T> adjugate() const  // °éËæ¡¢¹²éî¾ØÕó Adjugate matrix
	{
		mat<DimRows, DimCols, T> ret;
		for (size_t i = DimRows; i--; )
